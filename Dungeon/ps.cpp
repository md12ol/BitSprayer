...
void psDoc::setgray(int b) {//set a gray level of b
...
void psDoc::setblack() {//set the color to black
...
void psDoc::initfont() {          //loads times roman font size 12

    initfont(12);

}

void psDoc::initfont(int size) {  //loads TR font size "size"
...
void psDoc::initSfont(int size) {  //loads TR font size "size"
...
void psDoc::LeftLabel(const char *s) {//make a label at the left, centered
...
void psDoc::RightLabel(const char *s) {//make a label at the left, centered
...
void
psDoc::BottomLabel(int x, const char *s) {//make a bottom label, at x position
...
void psDoc::BottomLabel(const char *s) {//make a bottom label, centered
...
void psDoc::TopLabel(int x, const char *s) {//make a bottom label, at x position
...
void psDoc::TopLabel(const char *s) {//make a bottom label, centered
...
void psDoc::histPS(double *data,  //vector or reals to be histogrammed
                   ...
                   int tall) {     //max height - overridden if too small
...
//Make a filled polygon with a black border
void psDoc::nodepoly(int n, float h, float k, float r, float th,
                     int rr, int gg, int bb) {

    setcol(rr, gg, bb);      //get the fill color
    fillpoly(n, h, k, r, th);  //fill the polygon
    setblack();            //make the border color
    polygon(n, h, k, r, th);   //make the border
    ...
    if (vert) {
        ...
        for (i = 0; i < n; i++)
            if (flavor[i] > 0) {
                ...
                if (flavor[i] == 1)glyph(psn, val, 0, 3);
                else
                    glyph(psn,
                          val,
                          1,
